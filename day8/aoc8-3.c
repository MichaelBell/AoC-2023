// This is a less general but much faster solution to day 8 part 2,
// relying on the fact all the paths from start to end and end to end are
// a multiple of the instruction length, and all the ends loop to themselves
...
#define MAX_ENDS 6
end_route_t end_routes[MAX_ENDS] = {};
...
int main()
{
    ...
    for (int i = 0; i < num_ends; ++i) {
        create_end_route(0, end_to_node[i], &end_routes[i]);
        printf("%.3s %d %.3s\n", nodes[end_to_node[i]].name, end_routes[i].steps, nodes[end_to_node[end_routes[i].dest_route]].name);
    }

    // So in fact the paths to the ends and the loop at the end are the same length,
    // so the answer is just the LCM of the route lengths, and as the route lengths are all prime, this is just the product
    uint64_t sum = 1;
    for (int i = 0; i < num_ends; ++i) {
