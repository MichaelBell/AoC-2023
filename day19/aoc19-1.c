...
int main()
{
    ...
    while (*ptr) {
        ...
        int idx = start_rule;
        while (true) {
            bool rule_applies = (rules[idx].category == 0);
            if (!rule_applies) {
                int* val_ptr;
                switch (rules[idx].category) {
                    case 'x': val_ptr = &x; break;
                    case 'm': val_ptr = &m; break;
                    case 'a': val_ptr = &a; break;
                    case 's': val_ptr = &s; break;
                    default: fail();
                }
                if (rules[idx].lt) {
                    rule_applies = *val_ptr < rules[idx].threshold;
                }
                else {
                    rule_applies = *val_ptr > rules[idx].threshold;
                }
            ...
            if (rule_applies) {
                if (rules[idx].destination == ACCEPT) {
                    total += x + m + a + s;
                    break;
                }
                else if (rules[idx].destination == REJECT) {
                    break;
                }
                else {
                    if (rules[idx].destination < 0 || rules[idx].destination >= num_rules) fail();
                    idx = rules[idx].destination;
                }
            }
            else {
                ++idx;
