...
// Start points moved slightly as we can only have starts on a vertical.
#if 0
const char* data = "\
FF7F7F7F7F7F7F7F---7\n\
L|LJS|||||||||||F--J\n\
...
L7JLJL-JLJLJL--JLJ.L\n";
#else
const char* data = ".F-7F-..|-F-|F---7J7FLF-7F|FJ-LJ.7--F7-|77.|7J.FFLF7-FJ7.J-7-|.J.7.F7L7..FF.|F--F--|J-FFJFL7F7-LJFF-.7F|7.FF|7FF7FL7L--F--77-7F7.7.-F-J.JFLF\n\
...
LF|7-L|L-7L--7L7F7F---JL-JLJF-7F7L-7F--JF-J|||FJL7L7FJFJ|||F-J||L7|L7F-J|FJ|FJF7||F-JL7LJ|||L7FJ|||||L7L-7L7||F7F7LJL-------JL-J|FFF-JFJ|-7|\n\
LJJ|J|L7FJ-F7L7||||F7F7F---7|FJ||F7|L7F7L-7|||L7||FJL-J-LJ|L-7||FJ|FJ|F-JL7|L-J|||L7F-J|FJ|L7|L7|||||.L7FJFJ||S|||F7F-7F--------JF7L7FJJ||LL\n\
...
bool is_connected(char c, int dir) {
    switch (c) {
        case '|': return (dir == UP || dir == DOWN);
        case '-': return (dir == LEFT || dir == RIGHT);
        case 'L': return (dir == UP || dir == RIGHT);
        case 'J': return (dir == UP || dir == LEFT);
        case '7': return (dir == LEFT || dir == DOWN);
        case 'F': return (dir == RIGHT || dir == DOWN);
        case 'S': return true;
        case '.': return false;
...
const char* find_next(const char* ptr, int *dir) {
    if (!is_connected(*ptr, 2^*dir)) fail();
    switch (*ptr) {
        case '|': 
        case '-': break;
        case 'L': 
...
int main()
{
    ...
    for (dir = 0; ++dir; dir < 4) {
        const char* possible = move(start, dir);
        if (is_connected(*possible, dir^2)) {
            ptr = possible;
            printf("%c %d\n", *ptr, dir);
    ...
    int last_dir = dir;
    while (ptr != start) {
        const char* next_ptr = find_next(ptr, &dir);
        if ((dir == UP || dir == DOWN) && dir == last_dir) arena[ptr - data] = 'x';
        else arena[ptr - data] = '-';
        //printf("%c %d %d %c\n", *ptr, dir, last_dir, arena[ptr - data]);
        if (dir == UP || dir == DOWN) last_dir = dir;
        ptr = next_ptr;
        ++count;
    }
