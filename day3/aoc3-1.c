...
bool symbol_adjacent(int row, int start_col, int end_col) {
    int min_i = row == 0 ? 0 : row - 1;
    int max_i = row == num_rows - 1 ? row : row + 1;
    int min_j = start_col == 0 ? 0 : start_col - 1;
    int max_j = end_col == row_len - 1 ? end_col : end_col + 1;

    for (int i = min_i; i <= max_i; ++i) {
        for (int j = min_j; j <= max_j; ++j) {
            if (rows[i][j] != '.' && (rows[i][j] < '0' || rows[i][j] > '9')) {
                return true;
            }
        }
    }
...
int main()
{
    ...
    for (int i = 0; i < num_rows; ++i) {
        ...
        while (*ptr != '\n') {
            if (*ptr >= '0' && *ptr <= '9') {
                int val;
                const char* end = read_num(ptr, &val);
                if (symbol_adjacent(i, ptr - rows[i], end - 1 - rows[i])) {
                    sum += val;
                }
                ptr = end;
            }
